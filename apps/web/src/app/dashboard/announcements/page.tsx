...
export default function AnnouncementsPage() {
  const announcements = [
    ...
    },
    {
      ...
      slug: "holiday-schedule-update",
      thumbnail: false,
      date: "2026-09-15",
    },
  ];

  const [announcemen, setAnnouncement] = useState();
    ...
    const loadAnnouncements = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/announcement/get`,
        );
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A list of all published announcements.
          </CardDescription>
        </CardHeader>
        {loading ? (
          <Loader />
              ...
              <table className="w-full caption-bottom text-sm text-left">
                ...
                <tbody className="[&_tr:last-child]:border-0">
                  {announcements.map((item) => (
                    ...
                    >
                      <td className="px-6 py-4 align-middle font-semibold text-primary">
                        {item.title}
                      </td>
                      ...
                      <td className="px-6 py-4 align-middle">
                        <div className="flex gap-2 text-muted-foreground">
                          {item.thumbnail && <ImageIcon className="h-4 w-4" />}
                        </div>
                      </td>
                      <td className="px-6 py-4 align-middle text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 shadow-none hidden sm:flex"
                          >
                            Edit
                          </Button>
                          <Button
